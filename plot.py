...
for test_file in test_files:
    ...
    for model in reconstructed:
        ...
        ax.plot(df_rec['qt'], df_rec.depth, label = model, alpha = 0.3)

    ax.plot(df_orig['Smooth qt'], df_orig.Depth, label = 'Original')
    ...
    if not os.path.isdir(r'output/Model Evaluation/Sixteenth Attempt/plots/test/'):
        os.makedirs(r'output/Model Evaluation/Sixteenth Attempt/plots/test/')
    plt.savefig(r'output/Model Evaluation/Sixteenth Attempt/plots/test/' + str(test_file[0][:-4]) + '.pdf')
    plt.close()
...
reconst_model_train_dir = os.listdir('output/Model Evaluation/Sixteenth Attempt/')
...
for path in reconst_model_train_dir:
    files = os.listdir(r'output/Model Evaluation/Sixteenth Attempt/' + path)

    #create directory for the model output graphs for comparison
    if not os.path.isdir(r'output/Model Evaluation/Sixteenth Attempt/' + path):
        os.mkdir(r'output/Model Evaluation/Sixteenth Attempt/' + path)

    reconstructed[path] = {} #created nested multi-layered dicts
    for file in files:
        df = pd.read_csv(r'output/Model Evaluation/Sixteenth Attempt/' + path + '/'+ file)
        reconstructed[path][file] = df
...
for train_file in train_files:
    print(train_file[0])
        #load reformatted cpt data files
    #if str(orig_file) in str(test_files):
    df_orig = pd.read_csv('datasets/cpt_filtered_datasets/' + train_file[0])
    fig, ax = plt.subplots(figsize = (30,30))
    ...
    #    location.loc[location['CPT']==file[:-4], 'lat'].iloc[0],
    #    location.loc[location['CPT']==file[:-4], 'lng'].iloc[0]))
    if not os.path.isdir(r'output/Model Evaluation/Sixteenth Attempt/plots/train/'):
        os.makedirs(r'output/Model Evaluation/Sixteenth Attempt/plots/train/')

    plt.savefig(r'output/Model Evaluation/Sixteenth Attempt/plots/train/'  + str(train_file[0][:-4]) + '.pdf')
    plt.close()
...
# =============================================================================
# CPT plots for 09, 10, 11 to demonstrate the faulty end point data
# =============================================================================
