...
model_dir = os.listdir(r'Models/Third Attempt/Scaled/')
...
for ml_model in model_dir:
    model = tf.keras.models.load_model('/Users/goharshoukat/Documents/GitHub/synthetic-CPT/Models/Third Attempt/Scaled/' + ml_model)
    outdir = r'output/Model Evaluation/Third Attempt/test/' + ml_model
...
#using the same initial for loop.
    for file in train_data_dict:
        ...
        outdir = r'output/Model Evaluation/Third Attempt/' + ml_model
        if not os.path.isdir(outdir):
            os.mkdir(outdir)
        df2.to_csv(outdir + '/reconstructed_{}.csv'.format(file[:-4]), index = False)
